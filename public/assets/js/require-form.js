    ...
    var Form = {
        ...
        events: {
            ...
            faselect: function (form) {
                ...
                //绑定fachooseuser选用户件事件

                if ($(".fachooseuser", form).size() > 0) {

                    $(".fachooseuser", form).on('click', function () {

                        var that = this;

						var isd = $(that).attr("data-isd") ? $(that).attr("data-isd") : 0;

                        parent.Fast.api.open("user/user/selectuser?isd="+isd, __('Choose'), {

                            callback: function (data) {

                                var uid = $(that).attr("data-uid") ? $(that).attr("data-uid") : '';

                                var uname = $(that).attr("data-uname") ? $(that).attr("data-uname") : '';
